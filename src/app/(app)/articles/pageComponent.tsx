...
export default function PageComponent({
  ...
  const [generationStatus, setGenerationStatus] =
    useState<ArticleGenerationStatus | null>(null);
        ...
        <Tabs defaultValue='all'>
          <TabsList>
            ...
            <TabsTrigger value='generated'>
              Generated (
              {articles.filter((a) => a.generatingState === 'GENERATED').length}
              )
            </TabsTrigger>
            <TabsTrigger value='generating'>
              Generating (
              {
                articles.filter((a) => a.generatingState === 'GENERATING')
                  .length
              }
              )
            </TabsTrigger>
            <TabsTrigger value='draft'>
              Drafts (
              {articles.filter((a) => a.generatingState === 'DRAFT').length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value='generating' className='mt-4'>
            <ArticlesTable
              articles={articles.filter(
                (a) => a.generatingState === 'GENERATING'
              )}
              generationStatus={generationStatus}
              isRefreshing={isRefreshing}
              onRefresh={refreshArticles}
